...
- **`game-jam`** (`.claude/agents/game-jam.md`) — takes a theme or a named game and, without asking any clarifying questions, writes **two alternative specs** for the same game concept into `specs/game-jam/NN-slug.md` (its own sequential numbering, separate from `specs/`), each differing along one declared axis (fidelity, scope, mechanic, or category) and each with its own `id`. It reads `implemented-games.md` and `suggested-games.md` to avoid collisions but never writes to them. Like `game-planner`, it never writes code; the human picks one spec, promotes it into `specs/NN-slug.md`, flips its status to `Approved`, and runs `/spec-impl`.
- **`skin-designer`** (`.claude/agents/skin-designer.md`) — **the deliberate exception to "agents never write code."** It audits every game's `engine.ts`/`-player.tsx` for skin coverage and edits them directly (plus the shared `lib/skins.ts` / `components/skin-selector.tsx` it maintains) so each game offers at least the `clasico` (default), `neon`, and `retro` skins, legible against the site's permanently dark background. It doesn't go through `specs/` or `/spec-impl` — there's no spec-then-approve gate for it, since skin work is scoped, visual-only, and reversible per game.
- **`mobile-porter`** (`.claude/agents/mobile-porter.md`) — another deliberate code-writing exception, this one for touch/mobile support. It wires the pattern `specs/12-juegos-en-movil-tactil.md` established (shared `<TouchPad>`, `<HudMenu>`, `lib/canvas-hidpi.ts`, and pointer-drag where warranted) into **one explicitly named new game** added to the catalog after that spec. The four games spec 12 already migrated (`asteroids`, `tetris`, `arkanoid`, `snake`) are out of scope by design — they already work as expected.
- **`game-performance`** (`.claude/agents/game-performance.md`) — a third code-writing exception, this one for runtime performance. Given one game's `id`, explicitly named, it audits `engine.ts`/`-player.tsx` against the antipatterns `specs/14-rendimiento-cruce.md` diagnosed and fixed in `cruce` (a fully redrawn static background instead of a cached offscreen canvas, unstable-identity props defeating the `React.memo`-wrapped `<TouchPad>`/`<HudMenu>`/`<SkinSelector>`), measures with an automated Playwright session when a dev server is available, and applies the same fix pattern. Unlike `mobile-porter`, it has no legacy-game exclusion list — every game, including the four spec 12 already migrated, is a valid target.
