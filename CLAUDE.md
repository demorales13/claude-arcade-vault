...
- **`/spec-impl-game`** (`.claude/skills/spec-impl-game/SKILL.md`) — a wrapper for specs that add a playable game to the catalog. `/spec-impl` stays deliberately human-only (`disable-model-invocation: true`), so this command never invokes it as a skill; instead it reads `.claude/skills/spec-impl/SKILL.md` as text and carries out its documented phases itself. Once that reaches its own successful end, it automatically chains the `skin-designer` agent and then, once that reports back, the `mobile-porter` agent onto the game that was just implemented (skipping `mobile-porter` with a note if the game is one of the four spec 12 already migrated). Never runs the two agents in parallel — both edit the same `-player.tsx` file. Use it for game specs; use plain `/spec-impl` for anything else.
